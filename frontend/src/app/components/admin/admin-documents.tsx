...
export function AdminDocuments() {
  ...
  function DeleteDocButton({ doc, full = false }: { doc: ApiDocument; full?: boolean }) {
      ...
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Button
            ...
            variant="outline"
            disabled={removing}
            className={`text-destructive hover:text-destructive hover:border-destructive/40 ${full ? 'flex-1' : ''}`}
          >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              O documento <strong>{DOC_TYPE_LABELS[doc.type] ?? doc.type}</strong> de{' '}
              <strong>{driver.name}</strong> será removido permanentemente. Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive hover:bg-destructive/90" onClick={() => removeDocument(doc.id)}>
              Apagar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
